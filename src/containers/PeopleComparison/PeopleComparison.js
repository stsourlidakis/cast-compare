...
class PeopleComparison extends Component {
  ...
  searchSelect = (e) => {
    const person = this.state.autocompleteData.find(
  ...
  getPersonData = (personId) => {
    ...
    theMovieDB
      .get(`/person/${personId}?append_to_response=combined_credits`)
      .then((res) => {
        if (!this._isMounted) return;
  ...
  extractCredits = (person) => {
    ...
    const sortedCredits = uniqueCredits.sort((a, b) => {
      if (this.isCreditTalkShow(a)) {
        //move talkshows and documentaries at the end
        return 1;
  ...
  removePendingPerson = (personId) => {
    const newPendingPeople = this.state.pendingPeople
      .slice()
  ...
  updateCommonCredits = () => {
    ...
    const creditLists = this.state.people
      .map((p) => p.credits)
  ...
  render() {
    ...
    if (this.state.people.length > 1) {
      if (this.state.commonCredits.length > 0) {
        if (
          this.state.commonMoviesCounter > 0 &&
          this.state.commonShowsCounter > 0
        ) {
      ...
      <div className={styles.PeopleComparison}>
        ...
        <div className={styles.People}>
          ...
          {this.state.pendingPeople.map((person) => (
            <ComparedItem key={person.id} loading />
          ))}
        </div>
      </div>
    );
...
const PeopleComparisonWrapper = () => {
  ...
  return (
    <PeopleComparison params={params} navigate={navigate} location={location} />
  );
};
